...
async def read_player_data_in(message_sender, to_load, verbose=False):
        ...
        if len(player_data) > 0:
            try:
                ...
                if lookup_name in common.all_player_data:
                    await message_sender.queue_message(f"Warning: Duplicate player: {cur_player.name}")
                else:
                    common.all_player_data[lookup_name] = cur_player
            except Player.BadDataGiven:
                await message_sender.queue_message(f"This data received contains bad data: {player_data}")
    if verbose:
        await message_sender.queue_message(f"Read in {len(common.all_player_data)} players.\nData source (sheet or API) has a total of {len(to_load)} players.")