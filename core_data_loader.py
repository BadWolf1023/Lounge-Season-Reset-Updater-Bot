        ...
        if len(player_data) > 0:
            try:
                ...
                if lookup_name in common.all_player_data:
                    await message_sender.send_message(f"Warning: Duplicate player: {cur_player.name}", is_once_every_24_hr_message=True, alternative_ctx=alternative_ctx)
                else:
                    common.all_player_data[lookup_name] = cur_player
            except Player.BadDataGiven:
                await message_sender.send_message(f"This data received contains bad data: {player_data}", alternative_ctx=alternative_ctx)
    if verbose:
        await message_sender.send_message(f"Read in {len(common.all_player_data)} players.\nData source (sheet or API) has a total of {len(to_load)} players.", alternative_ctx=alternative_ctx)